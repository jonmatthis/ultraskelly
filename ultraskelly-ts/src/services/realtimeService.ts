...
export class RealtimeService {
    ...
    createSession(
        ...
        if (callbacks.onToolStart) {
            this.session.on('agent_tool_start', (event: ToolStartEvent): void => {
                callbacks.onToolStart?.(event.tool.name);
            });
        }

        if (callbacks.onToolEnd) {
            this.session.on('agent_tool_end', (event: ToolEndEvent): void => {
    ...
    createSessionWithAudioControl(
        ...
        // Audio streaming events with proper types
        this.session.on('audio_start', (event: AudioStartEvent): void => {
            console.log('🎵 Audio started:', event.itemId);
            callbacks.onAudioStart?.(event.itemId);
        });

        this.session.on('audio', (event: AudioEvent): void => {
            console.log('🎵 Audio chunk received:', event.data.byteLength, 'bytes');
            callbacks.onAudioChunk?.(event.data, event.itemId);
        });

        this.session.on('audio_stopped', (event: AudioStoppedEvent): void => {
