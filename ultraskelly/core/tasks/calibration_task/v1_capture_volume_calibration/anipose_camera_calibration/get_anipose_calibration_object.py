...
from ultraskelly.system.paths_and_filenames.path_getters import get_last_successful_calibration_toml_path
...
def load_most_recent_anipose_calibration_toml(
...
) -> ultraskelly_anipose.CameraGroup:
    ...
    try:
        camera_group = ultraskelly_anipose.CameraGroup.load(str(most_recent_calibration_toml_path))
...
def load_anipose_calibration_toml_from_path(
    ...
    try:
        anipose_calibration_object = ultraskelly_anipose.CameraGroup.load(str(camera_calibration_data_toml_path))
        if save_copy_of_calibration_to_this_path is not None:
            copy_toml_path = str(
...
def load_calibration_from_session_id(
    ...
    try:
        return ultraskelly_anipose.CameraGroup.load(str(session_calibration_file_path))
    except Exception as e:
